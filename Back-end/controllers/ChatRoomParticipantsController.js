...
export const createChatRoomForTwo = async (req, res) => {
	...
	try {
		...
		return res.status(StatusCodes.CREATED).json({
			message: 'Chat Room Created and Linked',
			id: chatRoom.id,
			Users: userNames,
			Messages: [],
		...
		return res
			...
			.send(`can\'t create chat room err: ${err}`);
	}
};
...
export const createChatRoom = async (req, res) => {
	...
	if (
		...
		receiverIds.some((id) => isNaN(id))
	) {
		return res
			.status(StatusCodes.BAD_REQUEST)
			.send('Invalid receiverIds');
	}
	const allUsers = [
		sender.id,
		...receiverIds.map((id) => Number.parseInt(id)),
	];
	...
	try {
		...
		for (const room of chatRooms) {
			const participants = room.Users.map(
				(user) => user.id
			);
			if (
