...
export default function ChangePass() {
  ...
  const handleNewPassChange = (event) => {
    ...
    setFormData((prevFormData) => ({ ...prevFormData, newPass: value }));

    if (value.length < 8) {
      setNewPassError('Input must be at least 8 characters');
      return;
    }

    if (!/[0-9]/.test(value)) {
      setNewPassError('Input must contain at least one number');
      return;
    }

    if (!/[@_#$]/.test(value)) {
      setNewPassError('Input must contain at least one special character');
      return;
    }
  ...
  const handleConfirmPass = (event) => {
    const value = event.target.value;
    setFormData((prevFormData) => ({ ...prevFormData, confirmPass: value }));

    if (formData.newPass !== value) {
      setConfirmPassError('Confirm password must equal to new password');
      return;
    }
    ...
    <span className={classes.root}>
      <div className={classes.container}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <NavLink className={classes.Cancel} to={HomeRoute}>
            Cancel
          </NavLink>
        </form>
      </div>
